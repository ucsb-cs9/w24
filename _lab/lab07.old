...
```python
cp1 = CustomPizza("L")
cp1.add_topping("extra cheese")
cp1.add_topping("sausage")
...
Unlike custom pizzas, specialty pizzas do not have a list of toppings associated with it, but do have a unique name that will be displayed when getting details for this pizza. This class should also have its own `get_pizza_details` method described below:

* `get_pizza_details(self)` - this method will construct and return a string containing the details of the `SpecialtyPizza` object including the size and name of the `SpecialtyPizza`. An example (with escape characters shown for formatting) is given below. When constructing your string, please follow the **EXACT** format since this is what Gradescope will expect

A sample output test for `get_pizza_details()`:
...
* `get_time(self)`
* `set_time(self, time)`
* `add_pizza(self, pizza)` - will add the Pizza object to the end of the Python List
* `get_order_description(self)` - constructs and returns a string containing the time of the order, all information for each pizza in the order, as well as the total order price. Since we're storing various Pizza objects in this class, we can utilize polymorphism and simply call the `get_pizza_details()` method on the Pizza objects when constructing the string for our entire order, as well as `get_price()` to compute the `PizzaOrder` total price

An example of the `get_order_description()` string format is given below:
...
assert order.get_order_description() == \
"******\n\
Order Time: 123000\n\
CUSTOM PIZZA\n\
Size: S\n\
...
* `add_order(self, pizzaOrder)` - a `pizzaOrder` object will be stored in the MinHeap *prioritized by its time attribute* (lower value means higher priority)
* `process_next_order(self)` - this removes the root node from the MinHeap (and restructures the MinHeap), and returns a string containing the root value's pizza order description

The automated tests will create various pizza orders with different time attributes. It will then call processNextOrder one at a time and check the removed PizzaOrder is in the right priority by checking their expected `get_orderD_description()` string. You should write similar tests to confirm the MinHeap state is in the correct order.
